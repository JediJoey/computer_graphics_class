...
{
	// increase or decrease movement speed. Scroll up = increase. Scroll down = decrease.
	g_pCamera->MovementSpeed += static_cast<float>(yOffset) * 0.5f;

	// set the movement speed changes
	if (g_pCamera->MovementSpeed < 0.1f) // Floor for speed changes to prevent infinite deceleration.
		g_pCamera->MovementSpeed = 0.1f;

	if (g_pCamera->MovementSpeed > 20.0f) // Cieling for speed changes to prevent infinite acceleration.
